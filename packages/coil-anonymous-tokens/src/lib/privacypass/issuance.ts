...
import { GenerateNewTokens, countStoredTokens, storeNewTokens } from './tokens'
import { patternToRegExp } from './utils'
import {
  sec1EncodeToBase64,
  getCurvePoints,
  verifyProof,
 ...
 */
export function signReqCF(url, details) {
  ...
  const alreadyProcessed = url.searchParams.has(
    requestIdentifiers()['post-processed']
  )
  ...
  const xhrInfo = {
    newUrl: newUrl,
    requestBody: `${bodyCaptcha}&blinded-tokens=${btRequest}`,
    tokens: tokens
  }
...
function signReqCFOld(reqUrl) {
  ...
  const xhrInfo = {
    newUrl: newUrl,
    requestBody: 'blinded-tokens=' + request,
    tokens: tokens
  }
 ...
 * @return {XMLHttpRequest} XHR info for asynchronous token issuance
 */
export function signReqHC(url, details) {
  const reqUrl = url.href
  ...
  const xhrInfo = {
    newUrl: reqUrl,
    requestBody: `blinded-tokens=${request}&captcha-bypass=true`,
    tokens: tokens,
    cancel: false
  }
  return xhrInfo
 ...
 */
export function sendXhrSignReq(xhrInfo, url, tabId) {
  ...
  xhr.onreadystatechange = function() {
    ...
    if (
      xhrGoodStatus(xhr.status) &&
      xhrDone(xhr.readyState) &&
      ...
      throw new Error(
        '[privacy-pass]: Cannot receive new tokens due to upper bound.'
      )
    }
  }
...
function validateResponse(url, tabId, data, tokens) {
  ...
  switch (signResponseFMT()) {
    ...
    default:
      throw new Error(
        '[privacy-pass]: invalid signature response format ' + signResponseFMT()
      )
  }

  if (signaturesJSON == null) {
    throw new Error(
      '[privacy-pass]: signature response invalid or in unexpected format, got response: ' +
        data
    )
  }
...
function validateAndStoreTokens(url, tabId, tokens, issueResp) {
  ...
  if (commitments) {
    if (!commitments.G || !commitments.H) {
      console.warn(
        '[privacy-pass]: stored commitments are corrupted: ' +
          commitments +
          ', version: ' +
          version +
          ', will retrieve via XHR.'
      )
    } else {
...
function createVerificationXHR(url, tabId, tokens, issueResp, version) {
  ...
  xhr.onreadystatechange = function() {
    if (xhrGoodStatus(xhr.status) && xhrDone(xhr.readyState)) {
      ...
      if (!commitments.G || !commitments.H) {
        throw new Error(
          '[privacy-pass]: Retrieved commitments are incorrectly specified: ' +
            commitments +
            ', version: ' +
            version
        )
...
function retrieveCommitments(xhr, version) {
  ...
  if (typeof cmt === 'undefined') {
    throw new Error(
      '[privacy-pass]: Retrieved version: ' + version + ' not available.'
    )
  }
